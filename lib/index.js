...
const proxy = targetObject => new Proxy(targetObject, {
  set: (target, property, value) => {
    ...
    if (property === 'notFound' && (typeof value !== 'function' || value.length < 2)) {
      targetObject.emit('error', new Error('A function with at least two parameters (req, res) is expected.'));
      result = false;
    }
...
class Router extends EventEmitter {
  constructor(options = {}) {
    ...
    this.notFound = (req, res) => res.status(404).send({
      status: 404,
